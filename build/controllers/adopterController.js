...
export default class AdoptionController {
    ...
    async generateAdopter(req, res) {
        try {
            const { name, mobile, picture, password, address } = req.body;
            if (!name || !mobile || !password) {
                return res.status(400).json({ error: "Campos obrigatórios: name, mobile, password" });
            }
            // address é objeto opcional
            const addressEntity = address && address.city && address.state
    ...
    async updateAdopter(req, res) {
        try {
            ...
            if (!Number.isFinite(id))
                return res.status(400).json({ error: "id inválido" });
            const { name, password, mobile, picture, address } = req.body;
            const patch = {};
            // campos simples
            ...
            if (typeof picture === "string")
                patch.picture = picture.trim() ? picture : null;
            if (picture === null)
                patch.picture = null;
    ...
    async updateAdopterAddress(req, res) {
        try {
            const id = Number(req.params.id);
            if (!Number.isFinite(id))
                return res.status(400).json({ error: "id inválido" });
